...
module Alg  -- Algebra
 (
  Sort,
  Sig(VbOp,ViOp,VaOp,IdOp,PopOp,P1Op,P2Op,IncOp,AddOp,AddInvOp,MultOp,SustrOp,ModOp,DivOp,EqualOp,
      MeOp,MiOp,AndOp,OrOp,NotOp,NewOp,UpdOp,EntryOp,DEntryOp,
      MuOp,DiOp,EqOp,SuOp,AdOp,NullOp,ConOp,DConOp,IsNullOp,HeadOp,TailOp,NOp,IOp),
  codop,stOp,editOp,editOpRa,editOpc,
...
data Sig = VbOp Bool  | ViOp Int |  VaOp [Int] |                                      
           ...
           NullOp  | ConOp | DConOp | IsNullOp | HeadOp | TailOp | NOp Int | IOp Int
 deriving (Show,Eq)
...
codop TailOp = "tail"
codop HeadOp = "head"
codop (NOp k)= "nod " ++ show k 
codop (IOp k)= "inp " ++ show k 

-- edición de operadores RAL
editOpRa IdOp  ss  q =  "id^" ++ q ++ "(" ++ head ss ++ ")" 
editOpRa (SuOp k)  ss  q =  "(-" ++ show k ++ ")^" ++ q ++ " "  ++ head  ss 
editOpRa (AdOp k)  ss  q =  "(+" ++ show k ++ ")^" ++ q ++ " " ++ head  ss 
editOpRa (MuOp k)  ss  q =  "(*" ++ show k ++ ")^" ++ q ++ " " ++ head  ss 
editOpRa (DiOp k)  ss  q =  "(%" ++ show k ++ ")^" ++ q ++ " " ++ head  ss
editOpRa (EqOp k)  ss  q =  "(==" ++ show k ++ ")^" ++ q ++ " " ++ head  ss  
editOpRa NewOp  ss  q =  "new^" ++ q ++ "(" ++ (head  ss) ++ ")"
editOpRa IncOp  ss  q =  "inc^" ++ q ++ "(" ++ (head  ss) ++ ")"
editOpRa IsNullOp  ss  q =  "isNull^" ++ q ++ "(" ++ (head  ss) ++ ")"
editOpRa HeadOp  ss  q =  "head^" ++ q ++ "(" ++ (head  ss) ++ ")"
editOpRa TailOp  ss  q =  "tail^" ++ q ++ "(" ++ (head  ss) ++ ")"
editOpRa P1Op  ss  q =  "p1^" ++ q ++ "(" ++ (head  ss) ++ "," ++ (head (tail  ss)) ++ ")"
editOpRa P2Op  ss  q =  "p2^" ++ q ++ "(" ++ (head  ss) ++ "," ++ (head (tail  ss)) ++ ")"
editOpRa EntryOp  ss  q =  (head  ss) ++ "[" ++ (head (tail  ss)) ++ "]^" ++ q 
editOpRa DEntryOp  ss  q = (head  ss) ++ "[" ++ (head (tail  ss)) ++ "," ++ s3 ++ "]^" ++ q  
 where s3 = if length ss < 3 then error ("Alg/editOp: no hay tres argumentos para .[.,.]") 
                             else head ((tail.tail) ss)                        
editOpRa (NOp k) ss  q = "nod " ++ show k
editOpRa (IOp k) ss  q = "inp " ++ show k
editOpRa ConOp  ss q =  "(" ++ (head  ss) ++ ":" ++ (head (tail  ss)) ++ ")^" ++ q 
editOpRa o  ss q =  case length  ss of 
                            0 -> codop o ++ "^" ++ q
                            1 -> "(" ++ codop o ++ (if o==NotOp then " " else []) ++ (head  ss) ++ ")^" ++ q 
                            2 -> "(" ++ (head  ss) ++ codop o ++ "^" ++ q ++ " " ++ (head (tail  ss)) ++ ")"
                            3 -> case o of
                                  UpdOp  -> (head  ss) ++ "[" ++ (head (tail  ss)) 
                                                                   ++ "<-" ++ (head ((tail.tail)  ss)) ++ "]^" ++ q  
                                  DConOp -> (head  ss) ++ "[" ++ (head (tail  ss)) 
                                                                   ++ ":"  ++ (head ((tail.tail)  ss)) ++ "]^" ++ q
                                  _      -> error ("editOpRa: " ++ codop o ++ show ss)                                 
                                  

-- edición de operadores tradicionales
...
editOp x (NOp k) ss = "nod " ++ show k
editOp x (IOp k) ss = "inp " ++ show k
-- el siguiente para términos lambda de códigos
editOp [] (ViOp k) (s:ss) = show k ++ "(" ++ editss (s:ss) ++ ")"
...
editOpc x (NOp k) ss = "nod " ++ show k
editOpc x (IOp k) ss = "inp " ++ show k
